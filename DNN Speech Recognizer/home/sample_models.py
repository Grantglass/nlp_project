...
def rnn_model(input_dim, units, activation, output_dim=29):
    ...
    time = TimeDistributed(Dense(output_dim))(bn_rnn)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
    model.output_length = lambda x: x
    print(model.summary())
...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time = TimeDistributed(Dense(output_dim))(rnn)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
...
def bidirectional_rnn_model(input_dim, units, output_dim=29):
    ...
    bi_rnn = Bidirectional(GRU(units, activation="relu",return_sequences=True, implementation=2, name="bi_rnn"))(input_data)
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time = TimeDistributed(Dense(output_dim))(bi_rnn)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
...
def final_model(input_dim, filters, kernel_size, conv_stride,
    ...
    bn_cnn_1d = BatchNormalization(name='bn_conv_1d')(conv_1d)
    
    rnn_1 = GRU(units, activation='relu',
        return_sequences=True, implementation=2, name='rnn_1')(bn_cnn_1d)
    ...
    rnn_norm = BatchNormalization(name="rnn_normalized")(time_dense)
    
    time = TimeDistributed(Dense(output_dim))(rnn_norm)
    
    dropout = Dropout(0.4) (time)
